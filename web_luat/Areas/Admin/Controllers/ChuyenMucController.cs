...
namespace web_luat.Areas.Admin.Controllers
{
    ...
    public class ChuyenMucController : Controller
    {
        ...
        {
            var list = db.tbl_ChuyenMuc.AsQueryable().Where(g => g.IsDelete == isTrash);
            if (!string.IsNullOrEmpty(Keyword))
            {
                list = list.Where(g => g.TenChuyenMuc.Contains(Keyword));
            }
            if (page == null) page = 1;
        ...
        [HttpPost]
        public JsonResult Create([Bind(Include = "Id,TenChuyenMuc")] tbl_ChuyenMuc banner)
        {
            try
            {
                if (string.IsNullOrEmpty(banner.TenChuyenMuc))
                {
                ...
                db.tbl_ChuyenMuc.Add(banner);
                db.SaveChanges();
                tbl_Log a = new tbl_Log();
                a.MoTa = "Thêm mới chuyên mục  " + banner.TenChuyenMuc;
                a.Controller = "ChuyenMuc";
                a.Action = "Create";
        ...
        {
            tbl_ChuyenMuc banner = db.tbl_ChuyenMuc.Find(id);
            return PartialView(banner);
        }
        [HttpPost]
        public JsonResult Edit([Bind(Include = "Id,TenChuyenMuc")] tbl_ChuyenMuc banner)
        {
            try
            {
                if (string.IsNullOrEmpty(banner.TenChuyenMuc))
                ...
                banner.IsDelete = false;
                db.tbl_ChuyenMuc.AddOrUpdate(banner);
                db.SaveChanges();
                tbl_Log a = new tbl_Log();
                a.MoTa = "Sửa chuyên mục  " + banner.TenChuyenMuc;
                a.Controller = "ChuyenMuc";
                a.Action = "Edit";
                a.NgayThucHien = DateTime.Now;
                a.IdUser = Convert.ToInt32(Session["Id"].ToString());
        ...
        public JsonResult Delete(int? id)
        {
            ...
            if (banner == null)
            {
                return Json(new { status = true, message = "Chuyên mục đã bị xóa." }, JsonRequestBehavior.AllowGet);
            }
            banner.IsDelete = true;
            db.tbl_ChuyenMuc.AddOrUpdate(banner);
            tbl_Log a = new tbl_Log();
            a.MoTa = "Xóa Chuyên mục ";
            a.Controller = "ChuyenMuc";
            a.Action = "Delete";
            a.NgayThucHien = DateTime.Now;
            a.IdUser = Convert.ToInt32(Session["Id"].ToString());
        ...
        public JsonResult DeleteTVC(int? id)
        {
            ...
            db.tbl_ChuyenMuc.Remove(banner);
            tbl_Log a = new tbl_Log();
            a.MoTa = "Xóa vĩnh viễn Chuyên mục ";
            a.Controller = "ChuyenMuc";
            a.Action = "DeleteTVC";
            a.NgayThucHien = DateTime.Now;
            a.IdUser = Convert.ToInt32(Session["Id"].ToString());
